...
public class NetworkSyncManager : MonoBehaviourPunCallbacks
{
    ...
    public bool[] GetUsedGuns()
    {
        ...
        foreach (Photon.Realtime.Player player in PhotonNetwork.PlayerList)
        {
            ...
            if (player.CustomProperties.TryGetValue("currGun",out _currGun))
    ...
    public Dictionary<string,int> GetScoreList()
    {
        ...
        {
            _scoreList[player.NickName] = (int)player.CustomProperties["score"];
            print(player.NickName + ((int)player.CustomProperties["score"]).ToString());
        }
